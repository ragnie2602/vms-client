...
class _PlayerTimelineState extends State<PlayerTimeline> {
  ...
  late final _time = ValueNotifier(widget.initialDate ?? DateTime.now().roundToSecond);
  Timer? _debounce;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (widget.initialDate != null) {
        _clampCentralDate(widget.initialDate!);
      }
    });
  }
  ...
  void _onHorizontalDragUpdate(double dx) {
    _clampCentralDate(_centralDate.value.subtract(Duration(milliseconds: dx.toInt() * _dragSpeed)));
  }
      ...
      builder: (context, state) {
        ...
        return Row(
          ...
          children: [
            Expanded(
              child: DecoratedBox(
                decoration: BoxDecoration(color: widget.backgroundColor ?? Color(0xFF3C3C3C)),
                child: LayoutBuilder(
                      ...
                      child: Listener(
                        onPointerSignal: (pointerSignal) {
                          if (pointerSignal is PointerScrollEvent) {
                            _onHorizontalDragUpdate(-pointerSignal.scrollDelta.dy);
                            _showOverlay(pointerSignal.localPosition.dx);
                          }
                        },
                        child: GestureDetector(
                          ...
                          onHorizontalDragUpdate: (data) => _onHorizontalDragUpdate(data.delta.dx),
