...
class MultiControlSpeedBox extends StatefulWidget {
  const MultiControlSpeedBox({super.key, required this.disabled});
  final bool disabled;

  @override
  State<MultiControlSpeedBox> createState() => _MultiControlSpeedBoxState();
}

class _MultiControlSpeedBoxState extends State<MultiControlSpeedBox> {
    ...
    showPopover(
      ...
      bodyBuilder: (context) => Column(
        ...
        children: _map.values
            .mapIndexed(
              (index, e) => _buildMenuItem(e, showTopBorder: index != 0),
            )
            .toList(),
      ),
  ...
  void _onItemTap(double value) {
    ...
    context.read<MultiPlaybackBloc>().add(MultiChangeSpeedEvent(value));
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: <Widget>[
            BlocSelector<MultiPlaybackBloc, MultiPlaybackState, double>(
  ...
  Widget _buildMenuItem(_SpeedData data, {showTopBorder = true}) {
    bool isSelected =
        data.value == context.read<MultiPlaybackBloc>().state.speed;
