...
class CustomPopupMenu extends StatefulWidget {
  const CustomPopupMenu({
    ...
    this.controller,
    this.arrowDecoration = const BoxDecoration(color: Color(0xFF4C4C4C)),
    this.showArrow = true,
    this.barrierColor = Colors.black12,
    this.arrowSize = const Size(10.0, 10.0),
    this.horizontalMargin = 10.0,
    this.verticalMargin = 10.0,
    this.position,
    this.menuOnChange,
    this.enablePassEvent = true,
    this.hideOnItemTap = true,
  });

  final Widget child;
  final PressType pressType;
  final bool showArrow;
  final Decoration arrowDecoration;
...
class _CustomPopupMenuState extends State<CustomPopupMenu> with SingleTickerProviderStateMixin {
  ...
  _showMenu() {
    Widget arrow = ClipPath(
      ...
      child: Container(
        ...
        decoration: widget.arrowDecoration,
      ),
    );
      ...
      builder: (context) {
        ...
        return Listener(
          ...
          onPointerDown: (PointerDownEvent event) {
            ...
            if (!widget.hideOnItemTap &&
                _menuRect.contains(Offset(offset.dx - widget.horizontalMargin, offset.dy))) {
              return;
            }
  ...
  void initState() {
    ...
    _animController = AnimationController(vsync: this, duration: Durations.long1);
...
enum _MenuPosition { bottomLeft, bottomCenter, bottomRight, topLeft, topCenter, topRight }
...
class _MenuLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(_MenuLayoutId.content)) {
      contentSize = layoutChild(_MenuLayoutId.content, BoxConstraints.loose(size));
    }
    ...
    switch (menuPosition) {
      ...
      case _MenuPosition.bottomLeft:
        arrowOffset = Offset(anchorCenterX - arrowSize.width / 2, anchorBottomY + verticalMargin);
        contentOffset = Offset(0, anchorBottomY + verticalMargin + arrowSize.height);
        break;
