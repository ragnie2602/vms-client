...
class ExportFileUserCase {
  static Future<void> exportExcel(List<CameraEntity> cameras) async {
    ...
    final headers = originalHeaders
        .where(
          (cell) => (cell?.value?.toString().toLowerCase() ?? '') != 'password',
        )
        .toList();
    headers.removeWhere((element) => element?.value == null);

    int dataStartRow = 1;
    List<String> headerString = headers
    ...
    sheetObject.appendRow(
      headerString.map((cell) => TextCellValue(cell)).toList(),
    );
    ...
    for (var i = 0; i < cameras.length; i++) {
      ...
      List<CellValue> row = [
        ...
        TextCellValue(camera.stream.userOriginAddedUrl),
